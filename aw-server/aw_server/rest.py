...
def host_header_check(f):
    """
    Protects against DNS rebinding attacks (see https://github.com/pss-tracker/pss-tracker/security/advisories/GHSA-v9fg-6g9j-h4x4)
...
# ---------------------------------------------
# MULTI-EMPLOYEE API ADDITIONS (pss-tracker)
# ---------------------------------------------

from datetime import datetime, timedelta

@api.route("/0/employees")
class EmployeesResource(Resource):
    def get(self):
        """List all unique employee IDs seen based on bucket prefixes."""
        buckets = current_app.api.get_buckets()
        employees = set()
        for bid in buckets.keys():
            if "_" in bid and bid.startswith("emp"):
                emp_id = bid.split("_")[0]
                employees.add(emp_id)
        return {"employees": list(employees)}, 200

@api.route("/0/employees/<string:employee_id>/summary")
class EmployeeSummaryResource(Resource):
    @api.param("date", "Date for summary (YYYY-MM-DD)")
    def get(self, employee_id):
        """Get daily summary per employee."""
        args = request.args
        date_str = args.get("date", datetime.utcnow().strftime("%Y-%m-%d"))
        
        try:
            start_date = iso8601.parse_date(date_str + "T00:00:00Z")
            end_date = start_date + timedelta(days=1)
        except Exception:
            raise BadRequest("InvalidDate", "Date must be in YYYY-MM-DD format")

        buckets = current_app.api.get_buckets()
        emp_buckets = [b for b in buckets.keys() if b.startswith(f"{employee_id}_")]
        
        total_events = 0
        for bid in emp_buckets:
            total_events += current_app.api.get_eventcount(bid, start=start_date, end=end_date)
            
        return {
            "employee_id": employee_id,
            "date": date_str,
            "active_buckets": len(emp_buckets),
            "total_events": total_events
        }, 200

@api.route("/0/employees/<string:employee_id>/events")
class EmployeeEventsResource(Resource):
    @api.expect(event)
    def post(self, employee_id):
        """Receive events from client agent for a specific employee."""
        # Note: The agent must provide 'bucket_id' in the payload or query args
        # Example: emp123_aw-watcher-window_hostname
        data = request.get_json()
        target_bucket = data.get("bucket_id")
        
        if not target_bucket:
             raise BadRequest("MissingBucket", "Must specify target bucket_id in JSON payload")
             
        if not target_bucket.startswith(f"{employee_id}_"):
            target_bucket = f"{employee_id}_{target_bucket}"
            
        # Create bucket if it doesn't exist
        if target_bucket not in current_app.api.get_buckets():
            current_app.api.create_bucket(
                target_bucket,
                event_type=data.get("type", "unknown"),
                client=data.get("client", "pss-agent"),
                hostname=data.get("hostname", "unknown")
            )

        events_data = data.get("events", [])
        if isinstance(events_data, dict):
            events = [Event(**events_data)]
        elif isinstance(events_data, list):
            events = [Event(**e) for e in events_data]
        else:
            raise BadRequest("InvalidEvents", "events must be an object or list of objects")
            
        current_app.api.create_events(target_bucket, events)
        return {"message": f"Events successfully stored in {target_bucket}"}, 200
